...
async def _(event):
    ...
    await tbot.edit_message(
        chatid,
        msgid,
        "Lol I found some torrents for you. Take a look 👇",
        buttons=[
            [
                Button.inline(
...
async def paginate_prevtorrent(event):
    ...
    try:
        ...
        await event.reply("Sorry, I couldn't find any torrents for that word")
        print(e)
        return
    vector = len(results)
    if num < 0:
...
async def torrentstop(event):
    ...
    await tbot.edit_message(
        chatid,
        msgid,
        "Thanks for using.\n❤️ from [Count Dracula](t.me/CountDracula_bot) !",
        link_preview=False,
    )
